...
(function () {
  ...
  //config
  cardCount = 12, // for faster test win
  ...
  cardTemplate = $cardContainer.html(),
  ...
  life,
  $lifeContainer = $('.memory-game__life-container'),
  $lifes,
  lifeTemplate = $lifeContainer.html(),

  makeLifes = function () {
    console.log('makeLifes');

    var lifesHtml = '';

    for(var i=0; i<lifeCount; i++) {
      lifesHtml += lifeTemplate;
    }

    $lifeContainer.html(lifesHtml);

    $lifes = $('.memory-game__life')
  ...
  setEvents = function () {
    ...
    $('.memory-game__start-btn').click(function(){
      $('.memory-game__start-page').removeClass('active');
    });

    $('.memory-game__restart-btn').click(function () {
      restartGame();
      $('.memory-game__page').removeClass('active');
    })
  },
  ...
  win = function () {
    console.log('win');
    $('.memory-game__win-page').addClass('active');
  },

  lose = function () {
    console.log('lose');
    $('.memory-game__loose-page').addClass('active');
  },

  updateLife = function () {
    console.log('updateLife');

    for (var i=life; i<lifeCount; i++) {
      console.log(i);
      $lifes.eq(i).hide();
    }
  },
